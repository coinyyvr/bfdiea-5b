class Character {
	constructor(
		tid, tx, ty, tpx, tpy, tvx, tvy, tonob,
		tdire, tcarry, tcarryObject, tcarriedBy, tlandTimer, tdeathTimer, tcharState,
		tstandingOn, tstoodOnBy, tw, th, tweight, tweight2, th2,
		tatEnd, tfriction, tfricGoal, tjustChanged, tspeed, tbuttonsPressed, tpcharState,
		tsubmerged, ttemp, theated, theatSpeed
	) {
		this.id = tid;
		this.x = tx;
		this.y = ty;
	...
	applyForces(grav, control, waterUpMaxSpeed) {
		...
		if (!this.onob && this.submerged !== 1) {
			this.vy = Math.min(this.vy + gravity, 25);
		}
...
// Bind the class definition to the global window space so the timeline frames can read it
